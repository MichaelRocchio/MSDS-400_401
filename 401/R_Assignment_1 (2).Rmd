...
```{r test1a}
to_rep<-c(2, -5.1, -23)
df1<-c(0,1,2,3,4,13,rep(to_rep,3),sum(7/42, 3, 35/42))
df1
...
#from here i was a bit confused by the instructions and read it two ways. One way I am reading that i need to add the sum of df1 to df2.Another is that I add the vector values by index to create a new vector. I did both :)
df_sum<-sum(df1, df2)
df_sum

df_sum1<-rbind(df1,df2)
df_sum1<-colSums(df_sum1)
df_sum1
...
df3<-c(head(df1,1),tail(df1,1))
df3
df4<-setdiff(df1,df3)
df4
#did you mean unique elements? I am assuming no because we got the sum earlier and I believe this is the standard way to find elements in one vector but not the other.
```
...
```{r test1d}
df5<-sort(c(df3,df4),decreasing=FALSE)
df5
df5_sum<-round(sum(df5),2)
df5_sum
```
...
```{r test2a}
sec2_funct<- function(x) {
  y=sinpi(x/2)+cospi(x/2)
  return(y)
}
sec2_funct(2)
...
```{r test2c}
plot(x,y, main='y = sin(x/2) + cos(x/2) Graphed')
text(x=0.500,y=1.414, '[0.5,1.414]', cex=.8, pos=1)
points(x=0.500,y=1.414, pch='o', cex=1.5)
```
...
test<-intersect(xy,xy1)
test
plot(xy, main='Intersection Graph', col='blue', ylim=c(-2,2), xlim=c(-2,2))
lines(xy1, col='red')
text(x=2,y=-1, '[2,-1]', cex=.8, pos=1)
points(x=2,y=-1, pch='o', cex=1.5)
```
...
which(trees[,2]==76, trees[,1]==12.9, trees[,3]==24.2)
print(trees[12,])
print(trees[13,])
```
...
area=(r**2)*pi
stem(r)
hist(r, col="darkmagenta")
plot(x=r, y=area, pch = 19, col='blue')
text(x=r, y=area,r, cex=.71, pos=3)
...
```{r test3c}
boxplot(area, col='blue', notch=TRUE, horizontal = TRUE, main = "Areas caculated based on radii", xlab='Area')
...
```{r test3d}
##I thought coef =2 showed extreame outliers too
boxplot.stats(area, coef=2)
extreme <- boxplot.stats(x = trees$Volume)[["out"]] > (quantile(trees$Volume, .75) + 3.0*IQR(area))
head(trees)
trees['r']=(trees[,1])/2
trees['area']=(r**2)*pi
which(trees['area']==apply(trees['area'],2,max))
print(trees[31,1:3])
```
...
par(mfrow=c(2,2))
hist(x,col=3)
boxplot(x,col=3, main='y Boxplot', xlab='x', horizontal=TRUE)
hist(y,col=3)
boxplot(y,col=3, main='y Boxplot', xlab='y', horizontal=TRUE)
```
...
qqnorm(y, col=3)
qqline(y,col=3)
extreme_x <- boxplot.stats(x = x)[["out"]] > (quantile(trees$Volume, .75) + 3.0*IQR(x))
extreme_y <- boxplot.stats(x = y)[["out"]] > (quantile(trees$Volume, .75) + 3.0*IQR(y))
print(extreme_x)
print(extreme_y)
```
